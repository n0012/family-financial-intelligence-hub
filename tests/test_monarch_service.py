...
class TestMonarchServiceAuth(unittest.TestCase):
    ...
    @patch("app.monarch_service.resolve_secret")
    @patch("app.monarch_service.MonarchMoney")
...
class TestMonarchSyncIngestion(unittest.TestCase):
    @patch("app.monarch_service.bigquery.Client")
    def test_sync_all_accounts_and_categories(self, mock_bq_class):
        ...
        mock_client.get_transaction_categories = AsyncMock(
            return_value={
                "categories": [
                    {
                        "id": "cat_1",
