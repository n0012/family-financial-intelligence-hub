...
class TestSessionHistory(unittest.TestCase):
    ...
    def test_save_session_history_persists_and_caches(self):
        ...
        self.assertIn("chat_history", args[0])
        row = args[1][0]
        self.assertEqual(row["session_id"], "spaces/SPACE_1/threads/THREAD_3")
        self.assertEqual(row["user_email"], "user@example.com")
        ...
        # Retrieve it back from in-memory cache without BigQuery query call
